...
def login():
    ...
    if usuario == "" or password == "":
        ...
        url = "http://www.seriecanal.com/index.php?page=member&do=login&tarea=acceder"
        #url = httptools.downloadpage(url, post=post).data
        url = httptools.downloadpage(url, post=post).data
        #url1 = url.headers.get("location", "")
        logger.info("Intel11 %s" %url)
        data = httptools.downloadpage("https://www.seriecanal.com/index.php?page=member&do=member&mensa=2").data
...
def mainlist(item):
    logger.info()
    itemlist = []
    d = httptools.downloadpage("http://www.wikiseriesypelis.com/")
...
def findvideos(item):
    ...
    logger.info("Intel11 %s" %data_download)
