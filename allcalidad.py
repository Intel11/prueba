...
def mainlist(item):
    ...
    itemlist.append(Item(channel = item.channel, title = "Por género", action = "generos", url = host + "api/rest/listing?post_type=movies&posts_per_page=16", thumbnail = get_thumb("genres", auto = True) ))
...
def search(item, texto):
    logger.info()
    texto = texto.replace(" ", "+")
    item.url = host + "search/" + texto + "/?page="
...
def generos(item):
    ...
    logger.info("Intel11 %s" %sub)
    j = jsontools.load(sub)
    
    for genero in j:
        #jj = jsontools.dump(genero)
        #logger.info("Intel33 %s" %jj)
        #logger.info("Intel44 %s" %genero)
        #logger.info("Intel55 %s" %j[genero]["slug"])

        url = host + "genero/" + j[genero]["slug"]
...
def peliculas(item):
    ...
    item.busca = ""
    dd = httptools.downloadpage(item.url + "%s" %item.pagina, encoding=encoding, canonical=canonical).data
    logger.info("Intel66 %s" %dd)
